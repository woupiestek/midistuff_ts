...
import { Nodes as Parser } from "./parser.ts";
import { Tokens } from "./tokens.ts";

Deno.test(function parseRest() {
  assertEquals(
    new Parser(new Tokens("_/4 r")).toString(),
    "0:DURATION\n1:  REST",
  );
});
...
Deno.test(function parseSimpleRest() {
  assertEquals(
    new Parser(new Tokens("r")).toString(),
    "0:REST",
  );
});
...
Deno.test(function parseNote() {
  assertEquals(
    new Parser(new Tokens("_3/4 -7++")).toString(),
    "0:DURATION\n1:  INTEGER_PLUS_PLUS",
  );
});
...
Deno.test(function parseSimpleNote() {
  assertEquals(
    new Parser(new Tokens("-7")).toString(),
    "0:INTEGER",
  );
});
...
Deno.test(function parseSet() {
  assertEquals(
    new Parser(new Tokens("_/4[ _/8 0 1 2 0 _/8 r ]")).toString(),
    "\
0:DURATION\n\
1:  LEFT_BRACKET\n\
2:    DURATION\n\
