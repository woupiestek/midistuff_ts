...
Deno.test(function durations() {
  assertEquals(new Interpreter(new Parser("_13/16[0]").parse()).realTime, 1625);
  assertEquals(
    new Interpreter(new Parser("[_13/16 0]").parse()).realTime,
    1625,
  );
});

Deno.test(function simpleChord() {
  const messages = new Interpreter(new Parser("_/2{ 0 2- 4 }").parse())
    .messages;
...
Deno.test(function simpleRepeat() {
  const messages = new Interpreter(new Parser("[$C = _/4 0 $C]").parse())
    .messages;
